...
- **What it does**: Adds `horizon-scanner.monitor-review` to the deterministic alert review table.
- **Why it matters**: Shows source status, affected products, owner, proof-packet status, delivery blockers and next action for each alert row.
...
- **Output**: Rows include review cells, pinpoint citations, reviewer decisions and blocked delivery status.
