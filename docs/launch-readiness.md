...
No legal, client, external communication should be sent automatically. Public-source classification may be AI-assisted only when configured explicitly and only for public publication text. Product-map impact scoring should remain deterministic and local unless a specific governance decision changes that design.
